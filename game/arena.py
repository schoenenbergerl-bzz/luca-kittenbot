...
class Arena:
    ...
    def start_round(self, bot_count: int) -> None:
        """
    ...
    def analyze_turn(self, response) -> bool:
        ...
        if self._state == 'PLAY':
            ...
            if 0 <= response <= len(self._deck):
                self._deck.insert(response, Card(CardType.EXPLODING_KITTEN))
            else:
    ...
    def _has_card(self, cardname) -> bool:
        """
        Check if the bot has this card.
        :param cardname: the name of the card
        :return: True if the play is legal, False otherwise
        """
        if cardname in self._bots_alive[self._active_bot].hand:
            return True
        return True
